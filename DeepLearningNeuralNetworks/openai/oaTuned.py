...
def createNextGenerationData(requirement, model):

    new_data = []


    while len(new_data) < 102400:
...
# print(model)
# averageScore = scoreModel(model)

# # playModel(model)
# training_data = createNextGenerationData(averageScore, model)
# newmodel = train_model(training_data, model)
# print(newmodel)
# averageScore = scoreModel(newmodel)
# # playModel(model)

model = neural_network_model(4)
model.load('first.model')
training_data = np.load('saved.np.npy')
newmodel = train_model(training_data, model)
averageScore = scoreModel(newmodel)
