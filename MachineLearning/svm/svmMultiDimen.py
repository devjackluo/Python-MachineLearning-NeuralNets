...
class Support_Vector_Machine:
    ...
    def fit(self, data):
        ...
        # Done TODO figure out how to make dynamic transforms for multi dimension
                ...
                if w[0] < 0:
                    ...
                    if self.isThird:
                        ...
                        if self.stepIntervalRotation == 0:
                            ...
                            self.currentIsOne = True
                            self.isThird = False
                            wThree = wOne - step*int(self.constantSteps/4)

                    elif self.currentIsOne:
                        ...
                        if self.stepIntervalRotation == 0:
                            ...
                            #self.currentIsOne = True
                            self.isThird = True
                            wTwo = wThree - step*int(self.constantSteps/4)




                    w = np.array([wOne, wTwo, wThree])
    ...
    def visualize(self):
        [[self.ax.scatter(x[0], x[1], x[2], s=100, color=self.colors[i]) for x in data_dict[i]] for i in data_dict]
        ...
        self.ax.plot([hyp_x_min, hyp_x_max,hyp_x_max],[nsv1,nsv2,nsv2], 'r')
        ...
        self.ax.plot([hyp_x_min, hyp_x_max,hyp_x_max],[psv1,psv2,psv2], 'g')

        # (w.x+b) = 0
        # divider bound
        db1 = hyperplane(hyp_x_min, self.w, self.b, 0)
        db2 = hyperplane(hyp_x_max, self.w, self.b, 0)
        self.ax.plot([hyp_x_min, hyp_x_max,hyp_x_max],[db1,db2,db2], 'y--')
...
data_dict = {1:np.array([[6,5,11],[0,-1,7],[0,-2,4],[5,5,6]]),
             -1:np.array([[7,3,2],[4,2,0],[8,4,-2],[8,5,5]])}
